...
function print_usage() {
    echo ""
    echo "./convert_rinex.sh -i rinex_path -vi old_rinex_version -vo new_rinex_version [-h -v -t]"
    echo "  -i: Full path to folder containing RINEX files"
...
if [[ -z ${rinex_path:-} ]]; then
    log_err "Path to RINEX files not set (-i)"
    print_usage
    exit 1
fi
